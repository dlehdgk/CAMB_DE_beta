...
class TransferParams(CAMB_Structure):
    ...
    _fields_ = [
        ...
        (
            "k_per_logint",
            c_int,
            "number of points per log k interval. If zero, set an irregular optimized spacing",
        ),
...
class AccuracyParams(CAMB_Structure):
    ...
    _fields_ = [
        ...
        (
            "lAccuracyBoost",
            c_double,
            "Boosts number of multipoles integrated in Boltzmann hierarchy",
        ),
        (
        ...
        (
            ...
            c_double,
            "Accuracy of k sampling for source time integration",
        ),
        ...
        (
            "NonlinSourceBoost",
            c_double,
            "Accuracy of steps and kmax used for the non-linear correction",
        ),
        ...
        (
            "SourceLimberBoost",
            c_double,
            "Scales when to switch to Limber for source windows",
        ),
        ("KmaxBoost", c_double, "Boost max k for source window functions"),
        (
            "neutrino_q_boost",
            c_double,
            "Number of momenta integrated for neutrino perturbations",
        ),
    ]
...
class SourceTermParams(CAMB_Structure):
    ...
    _fields_ = [
        ...
        (
            "line_basic",
            c_bool,
            "Include main 21cm monopole density/spin temperature sources",
        ),
...
class CAMBparams(F2003Class):
    ...
    _fields_ = [
        ...
        (
            "WantTransfer",
            c_bool,
            "Calculate matter transfer functions and matter power spectrum",
        ),
        ...
        (
            "Want_cl_2D_array",
            c_bool,
            "For the C_L, include NxN matrix of all possible cross-spectra between sources",
        ),
        (
            "Want_CMB",
            c_bool,
            "Calculate the temperature and polarization power spectra",
        ),
        ...
        (
            "min_l",
            c_int,
            "l_min for the scalar C_L (1 or 2, L=1 dipoles are Newtonian Gauge)",
        ),
        ...
        (
            "max_eta_k",
            c_double,
            "Maximum k*eta_0 for scalar C_L, where eta_0 is the conformal time today",
        ),
        (
            "max_eta_k_tensor",
            c_double,
            "Maximum k*eta_0 for tensor C_L, where eta_0 is the conformal time today",
        ),
        ...
        (
            "num_nu_massive",
            c_int,
            "Total physical (integer) number of massive neutrino species",
        ),
        ...
        (
            "z_outputs",
            AllocatableArrayDouble,
            "redshifts to always calculate BAO output parameters",
        ),
        (
        ...
        (
            "OutputNormalization",
            c_int,
            "If non-zero, multipole to normalize the C_L at",
        ),
        (
            "Alens",
            c_double,
            "non-physical scaling amplitude for the CMB lensing spectrum power",
        ),
        (
            "MassiveNuMethod",
            c_int,
            {"names": ["Nu_int", "Nu_trunc", "Nu_approx", "Nu_best"]},
        ),
        (
            "DoLateRadTruncation",
            c_bool,
        ...
        (
            "Evolve_delta_Ts",
            c_bool,
            "Evolve the spin temperature perturbation (for 21cm)",
        ),
    ...
    def set_initial_power_table(
        self, k, pk=None, pk_tensor=None, effective_ns_for_nonlinear=None
    ):
        """
    ...
    def set_H0_for_theta(
        ...
        if not (0.001 < theta < 0.1):
            raise CAMBParamRangeError(
                "theta looks wrong (parameter is just theta, not 100*theta)"
            )
    ...
    def set_cosmology(
        ...
        omch2=0.12,
        omk=0.0,
        cosmomc_theta: Optional[float] = None,
        thetastar: Optional[float] = None,
        neutrino_hierarchy: Union[str, int] = "degenerate",
        num_massive_neutrinos=1,
        mnu=0.06,
        nnu=constants.default_nnu,
        YHe: Optional[float] = None,
        meffsterile=0.0,
        standard_neutrino_neff=constants.default_nnu,
        TCMB=constants.COBE_CMBTemp,
        tau: Optional[float] = None,
        zrei: Optional[float] = None,
        Alens=1.0,
        bbn_predictor: Union[None, str, bbn.BBNPredictor] = None,
        theta_H0_range=(10, 100),
        setter_H0=None,
    ):
        r"""
        ...
        neutrino_mass_fac = (
            constants.neutrino_mass_fac * (constants.COBE_CMBTemp / TCMB) ** 3
        )
        ...
        if (
            nnu >= standard_neutrino_neff
            or neutrino_hierarchy != neutrino_hierarchy_degenerate
        ):
        ...
        if omnuh2_sterile > 0 and nnu < standard_neutrino_neff:
            raise CAMBError(
                "sterile neutrino mass required Neff> %.3g" % constants.default_nnu
            )
        ...
        if omnuh2_sterile > 0:
            if nnu < standard_neutrino_neff:
                raise CAMBError(
                    "nnu < %.3g with massive sterile" % constants.default_nnu
                )
        ...
        if cosmomc_theta or thetastar:
            ...
            if H0 < 1:
                raise CAMBValueError(
                    "H0 is the value in km/s/Mpc, your value looks very small"
                )
            self.H0 = H0
    ...
    def N_eff(self):
        ...
        if self.share_delta_neff:
            ...
            return (
                sum(self.nu_mass_degeneracies[: self.nu_mass_eigenstates])
                + self.num_nu_massless
            )
    ...
    def set_dark_energy(
        self, w=-1.0, cs2=1.0, wa=0, beta_DE=1, dark_energy_model="fluid"
    ):
        r"""
    ...
    def get_Y_p(self, ombh2=None, delta_neff=None):
        ...
        try:
            ...
            delta_neff = (
                delta_neff
                if delta_neff is not None
                else self.N_eff - constants.default_nnu
            )
            return self.bbn_predictor.Y_p(ombh2, delta_neff)
        except AttributeError:
            raise CAMBError(
                "Not able to compute Y_p: not using an interpolation table for BBN abundances."
            )
    ...
    def get_DH(self, ombh2=None, delta_neff=None):
        ...
        try:
            ...
            raise CAMBError(
                "Not able to compute DH: not using an interpolation table for BBN abundances."
            )
    ...
    def set_matter_power(
        ...
        if nonlinear is not None:
            if nonlinear:
                ...
                if not silent and (kmax < 5 or kmax < 20 and np.max(zs) > 4):
                    logging.warning(
                        "Using kmax=%s with Halofit non-linear models may give inaccurate results"
                        % kmax
                    )
            else:
                if self.NonLinear in [NonLinear_lens, NonLinear_both]:
        ...
        if len(redshifts) > max_transfer_redshifts:
            raise CAMBError(
                "You can have at most %s redshifts" % max_transfer_redshifts
            )
    ...
    def set_for_lmax(
        ...
        if lens_potential_accuracy:
            ...
            self.max_eta_k = max(
                self.max_eta_k, lens_k_eta_reference * lens_potential_accuracy
            )
    ...
    def set_custom_scalar_sources(
        ...
        if isinstance(custom_sources, dict):
            ...
            if source_ell_scales and not isinstance(source_ell_scales, dict):
                raise CAMBValueError(
                    "source_ell_scales must be a dictionary if custom_sources is"
                )
            lst = []
            source_names = []
        ...
        custom_source_names = source_names or [
            "C%s" % (i + 1) for i in range(len(custom_sources))
        ]
        if len(custom_source_names) != len(custom_sources):
            raise CAMBValueError(
                "Number of custom source names does not match number of sources"
            )
        ...
        self.f_SetCustomSourcesFunc(
            byref(c_int(len(custom_sources))), byref(custom_source_func), scales
        )
    ...
    def clear_custom_scalar_sources(self):
        self.f_SetCustomSourcesFunc(
            byref(c_int(0)), byref(ctypes.c_void_p(0)), np.zeros(0, dtype=np.int32)
        )
